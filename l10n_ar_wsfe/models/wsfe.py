...
from openerp.addons.l10n_ar_wsfe.wsfetools.wsfe2_suds import WSFE2
...
class wsfe_config(models.Model):
    ...
    _defaults = {
        'company_id': lambda self, cr, uid, context=None:
            self.pool.get('res.users')._get_company(cr, uid, context=context),
    ...
    def create(self, cr, uid, vals, context):
        ...
        wsaa_ids = wsaa_obj.search(cr, uid,
                                   [('company_id', '=', vals['company_id'])],
                                   context=context)
        service_ids = service_obj.search(cr, uid, [('name', '=', 'wsfe')],
                                         context=context)
        if wsaa_ids:
            ta_vals = {
                'name': service_ids[0],
